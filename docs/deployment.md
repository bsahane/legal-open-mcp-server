...
2. **Or build manually:**
   ```bash
   podman build -t legal-mcp-server .
   podman run -p 5001:5001 --env-file .env legal-mcp-server
   ```
...
| **Source copy** | Copies `legal_mcp_server/` into `/app` (dev deps and tests are excluded) |
...
| **Entrypoint** | `CMD ["/app/.venv/bin/python", "-m", "legal_mcp_server.src.main"]` |
...
3. **Import validation:**
   ```bash
   podman run --rm legal-mcp-server python -c "import legal_mcp_server; print('OK')"
   ```
