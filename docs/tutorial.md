...
Create `legal_mcp_server/src/tools/greet_tool.py`:

```python
"""Greet tool for the Legal MCP Server.
...
Open `legal_mcp_server/src/mcp.py` and make two edits.

**Add the import** (with the other tool imports near the top):

```python
from legal_mcp_server.src.tools.greet_tool import greet_user
```
...
1. **Tool file** -- one function per file in `legal_mcp_server/src/tools/`.
...
| Tool documentation format reference | [Tools README](../legal_mcp_server/src/tools/README.md) |
