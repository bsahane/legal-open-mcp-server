...
2. **Clone the repository:**
   ...
   ```bash
   git clone https://github.com/redhat-data-and-ai/legal-mcp-server
   cd legal-mcp-server
   ```
...
6. **Run the server:**
   ...
   python -m legal_mcp_server.src.main

   # Or using uv to run directly
   uv run python -m legal_mcp_server.src.main
   ```
...
2. **Run tests with coverage reporting:**
   ...
   ```bash
   pytest --cov=legal_mcp_server --cov-report=html --cov-report=term
   ```
...
2. **Type checking with MyPy:**
   ...
   ```bash
   mypy legal_mcp_server/
   ```
3. **Docstring validation:**

   ```bash
   pydocstyle legal_mcp_server/ --convention=google
   ```
