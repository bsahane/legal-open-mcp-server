.PHONY: help install clean test lint format coverage pre-commit local container deploy undeploy deps corpus corpus-check
...
lint: ## Run ruff linter and mypy type checker
	...
	.venv/bin/ruff check .
	.venv/bin/mypy legal_mcp_server/
...
coverage: ## Run tests with coverage report (80% minimum)
	...
	.venv/bin/python -m pytest --cov=legal_mcp_server --cov-report=term-missing --cov-report=html --cov-fail-under=80
...
corpus: ## Build the bundled bare-Acts corpus under data/acts
	@if [ ! -d ".venv" ]; then \
		echo "Error: Virtual environment not found. Run 'make install' first."; \
		exit 1; \
	fi
	.venv/bin/python scripts/build_seed_acts.py
...
local: ## Start MCP server locally
	...
	@. .venv/bin/activate && python -m legal_mcp_server.src.main
...
deploy: ## Deploy to target (usage: make deploy openshift)
	@if [ "$(filter openshift,$(MAKECMDGOALS))" = "openshift" ]; then \
		...
		sed -i.bak "s|namespace: legal-mcp-server|namespace: $(NAMESPACE)|g" deployment/openshift/kustomization.yaml; \
		...
		oc start-build legal-mcp-server --from-dir=. --follow || (mv deployment/openshift/deployment.yaml.bak deployment/openshift/deployment.yaml 2>/dev/null; mv deployment/openshift/kustomization.yaml.bak deployment/openshift/kustomization.yaml 2>/dev/null; exit 1); \
		...
		oc get pods -l app=legal-mcp-server; \
		echo ""; \
		echo "Useful commands:"; \
		echo "  View logs: oc logs -l app=legal-mcp-server --tail=100"; \
		echo "  Get route: oc get route legal-mcp-server"; \
		echo "  Check status: oc get pods,svc,route -l app=legal-mcp-server"; \
	else \
...
undeploy: ## Remove deployment (usage: make undeploy openshift)
	@if [ "$(filter openshift,$(MAKECMDGOALS))" = "openshift" ]; then \
		...
		oc delete deployment,service,route,configmap,secret,pvc,buildconfig,imagestream -l app=legal-mcp-server 2>/dev/null || true; \
